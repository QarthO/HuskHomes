...
public final class V12111UniversalVersionImplementation implements UniversalVersionImplementation {
    ...
    public void initialize() {
        ...
        ServerLifecycleEvents.SERVER_STARTING.register(server -> UniversalImplementationContext.logger().info(
                "Universal Fabric 1.21.11 scaffold attached to {}",
                server.getVersion()
        ));
        ServerLifecycleEvents.SERVER_STOPPING.register(server -> UniversalImplementationContext.logger().info(
                "Universal Fabric 1.21.11 scaffold stopping"
        ));
        EmbeddedLegacyRuntimeLauncher.launch(
                "1.21.11",
                "META-INF/huskhomes/runtime/runtime-1.21.11.jar"
        );
    }
}
